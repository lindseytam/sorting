...
def quick_sort(xs, cmp=cmp_standard):
    ...
    def _qs_helper(xs, lo, hi):

        if lo < hi:
            p = _partition(xs, lo, hi, cmp=cmp)
            _qs_helper(xs, lo, p-1)
            _qs_helper(xs, p+1, hi)
            return xs

    return _qs_helper(xs, 0, len(xs)-1)


def _partition(xs, low, high, cmp=cmp_standard):
    i = (low - 1)  # index of smaller element
    pivot = xs[high]  # pivot

    for j in range(low, high):

        if (cmp == cmp_standard and xs[j] < pivot) or (cmp == cmp_reverse and xs[j] > pivot):
